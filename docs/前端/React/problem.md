...
`fiberRoot`引用了整个fiber树结构，通过它可以访问到当前的fiber树，类型为FiberRoot；

`rootFiber`代表的是根容器对应的Fiber节点，也是Fiber树最顶层的Fiber节点，类型为Fiber；

> 在同一个React应用下，`fiberRoot`只有一个，而 `rootFiber`可能有多个，fiberRoot的current指向rootFiber，而rootFiber的stateNode指向fiberRoot。
